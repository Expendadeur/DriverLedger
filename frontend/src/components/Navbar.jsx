...
const Navbar = () => {
    ...
    return (
        <nav className="fixed w-full z-50 px-6 py-8">
            <div className="max-w-7xl mx-auto glass-morphism rounded-3xl px-8 py-5 flex items-center justify-between border border-white/40 shadow-2xl relative">
                <div className="flex items-center space-x-12">
                    <Link to="/" className="flex items-center space-x-3 group text-decoration-none">
                        <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30 group-hover:rotate-12 transition-all duration-500">
                            <Zap className="text-white fill-current" size={24} />
                        </div>
                        <span className="text-2xl font-black tracking-tighter text-slate-800">Drive<span className="text-blue-600">Ledger</span></span>
                    </Link>
                ...
                <div className="flex items-center space-x-6">
                    ...
                    <Link to="/cart" className="relative px-5 py-3 bg-slate-100 text-slate-600 rounded-2xl hover:bg-blue-50 hover:text-blue-600 transition-all group flex items-center space-x-2">
                        ...
                        {getItemsCount() > 0 && (
                            <motion.span
                                initial={{ scale: 0 }}
                                animate={{ scale: 1 }}
                                className="absolute -top-2 -right-2 w-6 h-6 bg-blue-600 text-white text-xs font-black rounded-full flex items-center justify-center border-2 border-white shadow-lg"
                            >
                                {getItemsCount()}
                            </motion.span>
                        )}
                    ...
                    {account ? (
                        <div className="flex items-center space-x-4">
                            ...
                            <button onClick={logout} className="p-3 bg-slate-100 text-slate-500 rounded-2xl hover:bg-red-50 hover:text-red-500 transition-all">
                                <LogOut size={20} />
                            </button>
                        </div>
                    ) : (
                        <button
                            onClick={login}
                            className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-bold flex items-center space-x-3 hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/25 active:scale-95"
                        >
                            <User size={18} />
                    ...
                    <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="lg:hidden p-3 bg-white border border-slate-100 rounded-2xl text-slate-600 hover:bg-slate-50 transition-all">
